#visualização com aba + filtro por:
#   Região do país onde foi realizada a compra;
#   Ano em que foi realizada a venda;
#   Vendedores que realizaram as vendas de produtos.
#
...
# ano = '' if todos_anos else :  ano = st.sidebar.slider('Ano', 2020, 2023)
if todos_anos:
    ano = ''
else:
...
# dados  #->visualiza dados

#---------------------------------------------------------------------------
## criando as tabelas

#->tabelas de receita----------------------------aba1
#-dados mapa 
receita_estados = dados.groupby('Local da compra')[['Preço']].sum()

# receita_estados    #->visualiza dados
...
# receita_estados = dados[['Local da compra', 'lat', 'lon']] \
#                                        .merge(receita_estados, left_on = 'Local da compra', right_index = True) \
#                                        .sort_values('Preço', ascending = False)
...
# vendas_mensal = pd.DataFrame(dados.set_index('Data da Compra').groupby(pd.Grouper(freq = 'M'))['Preço'] \
#                                         .count()).reset_index()
vendas_mensal = dados.set_index('Data da Compra').groupby(pd.Grouper(freq = 'M'))['Preço'] \
                                          .count().reset_index()
...
## Gráficos   ->aba2  -Quantidade de vendas ----------------------------------
#->Gráficos vendas
...
with aba3:                                                              #Vendedores
    qtd_vendedores = st.number_input('Quantidade de vendedores', 2, 10, 5) #usuario digita - entre 2 e 10, começa com 5)
...
#mostra em todas as abas os dados bruto
# st.dataframe(dados)  